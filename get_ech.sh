...
while read domain; do 
  if dig type65 +noall +answer $domain @1.1.1.1 | grep ech >/dev/null
    ...
    else echo $(cat domains.json | jq ".rules[0].domain_suffix[0] += [\"$domain\"]") > domains.json && echo $domain >> domains_noech.txt && number_of_domains_noech=$((number_of_domains_noech + 1))
  fi
